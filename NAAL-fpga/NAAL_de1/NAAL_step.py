...
class naal_socket(object):
    def __init__(self, IP_addr,dimensions,type=1 ,command=0):
    ...
    def send(self,dt, vector_data,command=1):
...
class NAAL_UDPnetwork(object):
    def __init__(self, host_IP,remote_IP,in_demesion,out_demension):
        ...
        self.send =naal_socket(self.host_IP,self.out_demension,0)
        self.recv()
        self.send()
        initarr=[]
        for i in range(0,self.out_demension):
           initarr.append(0.0)
        tuple(initarr)
        self.send.send(0,initarr,1)
        self.send.set_command(1)
        self.recv.recv()
        print "board recv:";
